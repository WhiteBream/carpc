...
//		}
		...
		// Ask for multimonitor stuff
		IVMRMonitorConfig	*pMonitor;

		hr = pVmr->QueryInterface(IID_IVMRMonitorConfig, (LPVOID*)&pMonitor); 
		if(FAILED(hr)) 
		{
			MessageBox(NULL, TEXT("IID_IVMRMonitorConfig failed"), TEXT("Error"), MB_OK);
			pVmr->Release();
			return(hr);
		}

		DWORD vCount;
		VMRMONITORINFO	vMonitors[10];
		VMRGUID			vDefaultGUID;
		VMRGUID			vCurrentGUID;

		hr = pMonitor->GetAvailableMonitors(vMonitors, 10, &vCount);
		if(FAILED(hr)) 
		{
			MessageBox(NULL, TEXT("GetAvailableMonitors failed"), TEXT("Error"), MB_OK);
			pVmr->Release();
			pMonitor->Release();
			return(hr);
		}
		hr = pMonitor->GetDefaultMonitor(&vDefaultGUID);
		if(FAILED(hr)) 
		{
			MessageBox(NULL, TEXT("GetDefaultMonitor failed"), TEXT("Error"), MB_OK);
			pVmr->Release();
			pMonitor->Release();
			return(hr);
		}
		hr = pMonitor->GetMonitor(&vCurrentGUID);
		if(FAILED(hr)) 
		{
			MessageBox(NULL, TEXT("GetMonitor failed"), TEXT("Error"), MB_OK);
			pVmr->Release();
			pMonitor->Release();
			return(hr);
		}
		...
		// The VMR is not needed any more 
	    pVmr->Release();
		pVmr = NULL;
