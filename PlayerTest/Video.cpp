	...
	switch(message)
	{
		...
		case WM_LBUTTONDBLCLK:
			...
			if(sPlacem.showCmd == SW_SHOWMAXIMIZED)
			{
				...
				GetClientRect(my->pAi->hApp, &pRect);
				SetWindowPos(hWnd, NULL, 0, 0, pRect.right, pRect.bottom, SWP_NOACTIVATE | SWP_NOZORDER);
			...
			else if(my->vVideoActive)
			{
				...
				SetWindowPos(hWnd, NULL, pRect.left, pRect.top, pRect.right, pRect.bottom, SWP_NOACTIVATE | SWP_NOZORDER);
				ShowWindow(hWnd, SW_SHOWMAXIMIZED);

				/* Get the desired size of the buttons */
				HWND hStart;
				GetClientRect(hWnd, &pRect);
				RECT startRect = {0, pRect.bottom - my->pAi->vOffs, 0, 0};
				hStart = FindWindow(TEXT("CRPC_STRTBTTN"), NULL);
				if(hStart)
					GetWindowRect(hStart, &startRect);
				SetWindowPos(my->hBar, NULL, startRect.right, startRect.top, pRect.right - startRect.right, my->pAi->vOffs, SWP_NOACTIVATE | SWP_NOZORDER);
		...
		case WM_SHOWVIDEOWINDOW:
			...
			if(my->vVideoActive)
			{
				...
				{
					if(sPlacem.showCmd == SW_SHOWMAXIMIZED)	// not very likely
					{
						ShowWindow(hWnd, SW_SHOWNORMAL);
						SetWindowLong(hWnd, GWL_STYLE, (GetWindowLong(hWnd, GWL_STYLE) | WS_CHILD));
						SetWindowLong(hWnd, GWL_EXSTYLE, NULL);
