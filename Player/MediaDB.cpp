...
DWORD WINAPI QueryThread(LPVOID lpParam) 
{
    ...
    my->pAi->hDb->MediaDbQuery((LPTSTR)lpParam, GetDlgItem(my->hCbWnd, IDC_MDBLIST));
	SetCursor(vOldCur);
	GlobalFree(lpParam);
	...
	switch (message)
	{
			...
			switch(HIWORD(wParam))
			{
				...
				case EN_CHANGE:
					...
					if(my->pAi->hDb->MediaDbStop())
					{
						...
						{
							LPTSTR pQueryText = (LPTSTR)GlobalAlloc(GPTR, sizeof(bText));
							StrCpy(pQueryText, bText);
							if(!CreateThread(NULL, 0, QueryThread, pQueryText, 0, NULL))
							{
								HCURSOR vOldCur = SetCursor(LoadCursor(NULL, IDC_WAIT));  
		...
		case WM_TIMER:
			switch(wParam)
			{
				...
				case 2:
					KillTimer(hWnd, 2);
					SendMessage(hWnd, WM_COMMAND, EN_CHANGE << 16, 0);
					break;
			}
			break;
