...
const LPTSTR szMediaDbMutex = TEXT("Global\\mediadbquerymutex");

static BOOL SetSafeDataAccess(BOOL vSafe)
{
	...
	if(vSafe)
	{
		hMutex = CreateMutex(NULL, FALSE, szMediaDbMutex);
		if(hMutex)
		{
			if(WaitForSingleObject(hMutex, MUTEX_WAIT_TIME) != WAIT_TIMEOUT)
				vRet = TRUE;
			ReleaseMutex(hMutex);
			CloseHandle(hMutex);
			hMutex = NULL;
...
static INT QueryToListboxCb(LPVOID cbParam, INT colCount, LPSTR *colData, LPSTR *colNames)
{
	...
	if(pData->pDb->vMediaDbRunning == 2)
		return(1);
...
void Database::MediaDbQuery(LPTSTR pSearch, HWND hWnd)
{
	const CHAR sqlQuery[] = "SELECT %s WHERE title IN (SELECT id FROM Ttitle WHERE UPPER(val) LIKE \'%%%q%%\') OR "
							"artist IN (SELECT id FROM Tartist WHERE UPPER(val) LIKE \'%%%q%%\');";
	PParamStruct pCbParm;
	LPSTR        szError = NULL;
	CHAR         vInStr[17];
	LPSTR        sqlCmd;
	...
	if(vMediaDbRunning)
	{
		MessageBox(NULL, TEXT("Invalid call to MediaDbQuery"), TEXT("Database"), MB_OK);
		vMediaDbRunning = 0;
		return;
	}
	if(SetSafeDataAccess(TRUE))
	{
		vMediaDbRunning = 1;
		SetSafeDataAccess(FALSE);
	}
...
BOOL Database::MediaDbStop(void)
{
	BOOL vRet = FALSE;
	if(SetSafeDataAccess(TRUE))
	{
