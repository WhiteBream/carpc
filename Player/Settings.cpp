...
typedef struct _ConfigParam
{
	...
	union
	{
		...
		BOOL	(*pCallbackBool)(LPVOID pConfig, BOOL vWrite, BOOL vValue);
	} pType;
} ConfigParam;
...
BOOL AntennaMode(LPVOID pConfig, BOOL vWrite, BOOL vValue)
{
	CHAR vRsp[5];
	...
	{
		if(vValue)
			PipeIo("SIwI\xA0\x50\xFF\0", 8, vRsp, sizeof(vRsp));
		else
			PipeIo("SIwI\xA0\x50\0\0", 8, vRsp, sizeof(vRsp));
	}
	else
	{
		PipeIo("SIrI\xA0\x50", 7, vRsp, sizeof(vRsp));
		if(vRsp[0])
			return(TRUE);
	}
	return(FALSE);
...
static const ConfigParam sConfig[] = 
{
	...
	{	
		CT_YESNO | CT_FUNCTION, CONFIGTYPE_BOOL, 
		TEXT("Activate Vant ouput when tuner active"),
		TEXT("No electric antenna installed"), 
		L"Powered antennas or antenna amplifiers can be powered from the Vant output on White Bream mobile computers.", 
		(LPSTR)AntennaMode
	},
	{	
...
Settings::Show(BOOL yesNo)
{
	...
	if(yesNo)
	{
		...
		for(i = 0; sConfig[i].vStyle; i++)
		{
			...
			{
				if(sConfig[i].vStyle & CT_YESNO)
					vToggle = sConfig[i].pType.pCallbackBool((LPVOID)&sConfig[i], FALSE, FALSE);
			}
					...
					switch(LOWORD(wParam))
					{
						...
						case IDC_ENABLE:
							...
							if((LRESULT)pConfig != LB_ERR)
							{
								...
								else if((pConfig->vStyle & CT_FUNCTION) && (pConfig->vStyle & CT_YESNO))
								{
									if(pConfig->vStyle & CT_CONFIRM)
										if(MessageBox(hWnd, pConfig->pTextOn, TEXT("Are you really sure?"), MB_YESNO | MB_ICONEXCLAMATION | MB_APPLMODAL) == IDNO)
											return(0);
									pConfig->pType.pCallbackBool((LPVOID)pConfig, TRUE, TRUE);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_DELETESTRING, i, 0);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_INSERTSTRING, i, (LPARAM)pConfig->pTextOn);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_SETITEMDATA, (WPARAM)i, (LPARAM)pConfig);
									SendDlgItemMessage(hWnd, IDC_ENABLE, BM_SETCHECK, (WPARAM)BST_INDETERMINATE, 0);
							...
							if((LRESULT)pConfig != LB_ERR)
							{
								...
								else if((pConfig->vStyle & CT_FUNCTION) && (pConfig->vStyle & CT_YESNO))
								{
									if(pConfig->vStyle & CT_CONFIRM)
										if(MessageBox(hWnd, pConfig->pTextOn, TEXT("Are you really sure?"), MB_YESNO | MB_ICONEXCLAMATION | MB_APPLMODAL) == IDNO)
											return(0);
									pConfig->pType.pCallbackBool((LPVOID)pConfig, TRUE, FALSE);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_DELETESTRING, i, 0);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_INSERTSTRING, i, (LPARAM)pConfig->pTextOff);
									SendDlgItemMessage(hWnd, IDC_SETTINGS, LB_SETITEMDATA, (WPARAM)i, (LPARAM)pConfig);
									SendDlgItemMessage(hWnd, IDC_ENABLE, BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);
					...
					switch(LOWORD(wParam))
					{
						...
						case IDC_SETTINGS:
							...
							if((LRESULT)pConfig != LB_ERR)
							{
								...
								else if(pConfig->vStyle & CT_FUNCTION)
								{
									if(pConfig->vStyle & CT_YESNO)
										toggle = pConfig->pType.pCallbackBool((LPVOID)pConfig, FALSE, FALSE);
								}
								if(pConfig->vStyle & CT_YESNO)
								{
