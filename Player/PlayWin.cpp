...
IBaseFilter *pAudioDev = NULL;

HRESULT FindSoundDevice(IBaseFilter ** ppSrcFilter);
...
PlayWin::~PlayWin()
{
	...
	if(pAudioDev) pAudioDev->Release();
...
//		}
		...
		pEventEx->SetNotifyWindow((long)hCallBack, WM_DIRECTSHOW, NULL);
		pEventEx->Release();

		// Route the played stream to the front zone audio device
		if(pAudioDev)
		{
			hr = pGraph->AddFilter(pAudioDev, L"Front audio device");
			if(FAILED(hr))
			{
				MessageBox(NULL, TEXT("AddFilter(pAudioDev) failed"), TEXT("Error"), MB_OK);
				return hr;
			}
		}
...
static HRESULT FindSoundDevice(IBaseFilter ** ppSrcFilter)
{
	...
	UINT devCount = waveOutGetNumDevs();
	MMRESULT waveErr;

	waveErr = waveOutGetDevCaps(WAVE_MAPPER, &waveCaps, sizeof(WAVEOUTCAPS));
	//wprintf(waveCaps.szPname);

	for(cFetched = 0; cFetched < devCount; cFetched++)
	{
		waveErr = waveOutGetDevCaps(cFetched, &waveCaps, sizeof(WAVEOUTCAPS));
		//wprintf(waveCaps.szPname);
	}
...
{ 
	...
	// Set the window. 
    hr = pWcontrol->SetVideoClippingWindow(hwndApp); 
    if (SUCCEEDED(hr))
    {
		...
		if(SUCCEEDED(hr) && IsWindowVisible(hwndApp))
		{
			...
			pWcontrol->SetAspectRatioMode(VMR_ARMODE_NONE);
...
//	DWORD			vCapabilities;
	...
	{
		//Don't start yet, await unpause message from mainwindow
		if(*state == State_Running)
			if(pControl)
				pControl->Run();
		...
		case WM_DIRECTSHOW:
			if(my->pEvent)
			{
				while(my->pEvent->GetEvent(&lEvCode, &lParam1, &lParam2, 0) == S_OK)
				{
					...
					{
						//MessageBoxA(NULL, "complete", "test", MB_OK);
