...
//#define DEFAULTLINEINDEVICE		L"SoundMAX Digital Audio"	// On NW8240
...
//#define DEFAULTLINEOUTDEVICE		L"DirectSound: SoundMAX Digital Audio"
#define DEFAULTLINEOUTDEVICE		L"DirectSound: USB Audio CODEC"
//#define DEFAULTLINEOUTDEVICE		L"DirectSound: Realtek AC97 Audio"
...
Radio::Radio(PAppInfo pAppInfo)
{
	...
	x += w;
	CreateWindow(WBBUTTON, TEXT("FM"), WS_CHILD | WS_VISIBLE, x, 0, (4 * offset) - x, row1, hWnd, (HMENU)IDC_BANDFM, hInst, NULL);
...
void Radio::OpenTuner(void)
{
	...
	else
	{
		...
		else
		{
			...
			WriteRegister(2, 0xFFFF, DISABLE);
			if(vTunersel == '1')
				WriteRegister(2, 0xFFFF, DSMUTE | ENABLE);
			else
...
IBaseFilter *pSrc = NULL, *pDest = NULL, *pWavDest = NULL, *pWriter = NULL, *pAsfDest = NULL;
...
HRESULT ConnectFilters1(IGraphBuilder *pGraph, IPin *pOut, IBaseFilter *pDest)
{
...
static HRESULT FindRadioInputDevice(IBaseFilter ** ppSrcFilter)
{
...
#endif
				if(((StrNCmp(DEFAULTUSBLINEINDEVICE, varName.bstrVal, 12) == 0) && (my->vTunersel == '2')) ||
					((StrNCmp(DEFAULTAC97LINEINDEVICE, varName.bstrVal, 12) == 0) && (my->vTunersel == '1')) ||
					(StrCmp(L"SoundMAX Digital Audio", varName.bstrVal) == 0))
				{
    ...
    // Do NOT Release() the IBaseFilter, since it will be used
...
static HRESULT FindSoundDevice(IBaseFilter ** ppSrcFilter)
{
	...
	//		hr = pPropBag->Read(L"CLSID", &varName, 0);
			if(SUCCEEDED(hr))
			{
				...
				MessageBox(NULL, varName.bstrVal, L"sounddevice", MB_OK);
#endif
				if(StrCmp(DEFAULTLINEOUTDEVICE, varName.bstrVal) == 0)
				{
			//	MessageBox(NULL, varName.bstrVal, L"sounddevice", MB_OK);
					// Bind Moniker to a filter object
					hr = pMoniker->BindToObject(0, 0, IID_IBaseFilter, (void**)&pSrc);
...
//		}
	...
	//	AddFilterByCLSID(pGraph, CLSID_WMAsfWriter, L"AsfSave", &pAsfDest);

		// Set the file name.
	//	pWriter->QueryInterface(IID_IFileSinkFilter, (void**)&pSink);
	//	pSink->SetFileName(L"C:\\MyWavFile.wav", NULL);

	//	hr = pAsfDest->QueryInterface(IID_IServiceProvider, (void**)&pProvider)
	//	if (SUCCEEDED(hr))
	//	{
	//		hr = pProvider->QueryService(IID_IWMWriterAdvanced2, IID_IWMWriterAdvanced2, (void**)&pWMWA2);
	//		pProvider->Release();
	//	}
	//	pAsfDest->QueryInterface(IID_IWMAsfWriter, (void**)&pAsfFile);
	//	pAsfFile->SetFileName(L"C:\\MyWavFile.asf", NULL);

		// Connect the filters.
	//	ConnectFilters(pGraph, pSrc, pWavDest);
	//	ConnectFilters(pGraph, pWavDest, pWriter);
	//	ConnectFilters(pGraph, pSrc, pAsfDest);
...
void CloseStream(void)
{
	if(pControl) {pControl->Stop(); delete pControl; pControl = NULL;}
	if(pGraph) {delete pGraph; pSrc = NULL;}
	if(pSrc)	{delete pSrc; pSrc = NULL;}
	if(pDest)	{delete pDest; pDest = NULL;}
}
