...
MainWindow::MainWindow(HINSTANCE hInstance, HWND hParent)
{
	...
	BOOL vMirror = dBase->GetBool("CameraMirror");
	SetMirrorMode(vMirror);
	if(vMirror)
		SendDlgItemMessage(hWnd, IDC_PREV, BM_SETCHECK, BST_CHECKED, 0);
	else
		SendDlgItemMessage(hWnd, IDC_NEXT, BM_SETCHECK, BST_CHECKED, 0);
	...
	switch (message)
	{
		...
		case WM_COMMAND:
			if(HIWORD(wParam) == BN_CLICKED)
			{
				...
				{
					case IDC_PREV:
						if(SendDlgItemMessage(hWnd, IDC_PREV, BM_GETCHECK, 0, 0) == BST_UNCHECKED)
						{
							vMirrorCurrent = TRUE;
							my->dBase->SetConfig("CameraMirror", &vMirrorCurrent, CONFIGTYPE_BOOL);
					...
					case IDC_NEXT:
						if(SendDlgItemMessage(hWnd, IDC_NEXT, BM_GETCHECK, 0, 0) == BST_UNCHECKED)
						{
							vMirrorCurrent = FALSE;
							my->dBase->SetConfig("CameraMirror", &vMirrorCurrent, CONFIGTYPE_BOOL);
							SetMirrorMode(vMirrorCurrent);
							SendDlgItemMessage(hWnd, IDC_NEXT, BM_SETCHECK, BST_CHECKED, 0);
		...
		case WM_SIZE:
			...
			SetWindowPos(GetDlgItem(hWnd, IDC_PREV), 0, w + 0 * h, ourRect.bottom - h, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
			SetWindowPos(GetDlgItem(hWnd, IDC_NEXT), 0, w + 1 * h, ourRect.bottom - h, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
			SetWindowPos(GetDlgItem(hWnd, IDC_EXIT), 0, w + 2 * h, ourRect.bottom - h, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
