// WinMixer.cpp: implementation of the WinMixer class.
//
//////////////////////////////////////////////////////////////////////
...
#define WINCLASS	(LPTSTR)TEXT("CRGWNMX")

LRESULT CALLBACK WinMixerProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
...
WinMixer::WinMixer(PAppInfo pAppInfo) : Client(pAppInfo, WINCLASS, (WNDPROC)WinMixerProc, NULL)
{
	RECT	vRect;
	INT		X, Y, H, W, i;
	INT		textSize;
	...
	SendDlgItemMessage(hWnd, IDC_BALANCE, TBM_SETRANGE, TRUE, MAKELONG(-30, +30));
	...
	SendDlgItemMessage(hWnd, IDC_FADER, TBM_SETRANGE, TRUE, MAKELONG(-25, +25));
	...
	SendDlgItemMessage(hWnd, IDC_EQ1, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ2, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ3, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ4, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ5, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ6, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
	...
	SendDlgItemMessage(hWnd, IDC_EQ7, TBM_SETRANGE, TRUE, MAKELONG(-15, +15));
...
void WinMixer::PaintBkgnd(HDC hDc)
{
	Brush    * ourBgnd;
	Graphics * ourGraphics;
    RECT	vRect;
...
LRESULT CALLBACK WinMixerProc(HWND hWnd, UINT vMsg, WPARAM wParam, LPARAM lParam)
{
	WinMixer	*my;
	PAINTSTRUCT ps;
	HDC		hDc;
	...
	my = (WinMixer*)GetWindowLong(hWnd, GWL_USERDATA);
	...
	{
		case WM_NCCREATE:
			my = (WinMixer*)((LPCREATESTRUCT)lParam)->lpCreateParams;
...
SetControlPosition(HWND hWnd, INT vControl, INT vValue)
{
	...
	switch(vControl)
	{
		...
		case IDC_BALANCE:
			if(GetDlgItem(hWnd, vControl) != hMoving)
				SendDlgItemMessage(hWnd, IDC_BALANCE, TBM_SETPOS, TRUE, (LPARAM)vValue);
		...
		case IDC_FADER:
			if(GetDlgItem(hWnd, vControl) != hMoving)
				SendDlgItemMessage(hWnd, IDC_FADER, TBM_SETPOS, TRUE, (LPARAM)-vValue);
		...
		case IDC_RESET:
			...
			SendDlgItemMessage(hWnd, IDC_RESET, BM_SETCHECK, (vInpTxt[0] == (CHAR)0xFF ? BST_UNCHECKED : BST_CHECKED), 0);
			break;
		...
		case IDC_EQ7:
			if(GetDlgItem(hWnd, vControl) != hMoving)
				SendDlgItemMessage(hWnd, vControl, TBM_SETPOS, TRUE, (LPARAM)-vValue);
			if(vValue > 0)
				StringCbPrintf(vText, sizeof(vText), TEXT("+%ddB"), vValue);
			else
				StringCbPrintf(vText, sizeof(vText), TEXT("%ddB"), vValue);
