...
LRESULT CALLBACK SpectrumProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_CREATE:
			...
			ti->vOurPos.left = cs->x;
			ti->vOurPos.top = cs->y;
			ti->vOurPos.right = cs->x + cs->cx;
			ti->vOurPos.bottom = cs->y + cs->cy;

			SHGetFolderPath(NULL, CSIDL_PROGRAM_FILES_COMMON, NULL, SHGFP_TYPE_CURRENT, vPath);
			PathAppend(vPath, TEXT("White Bream\\Mixer.dll"));
			ti->hMixer = LoadLibrary(vPath);
			if(ti->hMixer)
			{
				HFNCB dllIds;
				dllIds = (HFNCB)GetProcAddress(ti->hMixer, "SetSpectrum");
				if(dllIds)
					(dllIds)(hWnd);
			}
			break;
		...
		case WB_SETALPHA:
			...
			ti->back = new Color((BYTE)lParam, 255, 255, 255);
			ti->white = new SolidBrush(Color((BYTE)lParam, 255, 255, 255));
			break;
		...
		case WM_DESTROY:
			if(ti->hMixer) FreeLibrary(ti->hMixer);
		...
		//	hDc = GetDC(ti->hParent);
		//	BitBlt(ti->hdc, 0, 0, ti->wr.right, ti->wr.bottom, hDc, ti->vOurPos.left, ti->vOurPos.top, SRCCOPY);
		//	ReleaseDC(ti->hParent, hDc);
			ti->g->Clear(*ti->back);
			//ti->g->FillRectangle(ti->white, 0, 0, ti->wr.right, ti->wr.bottom);
		...
		case WB_SPECTRUM:
			...
			vSpectrum.l[0] = (UINT)wParam;
			vSpectrum.l[1] = (UINT)lParam;
			// Do the displaying part
			for(i = 0; i < BARCOUNT; i++)
			{
				INT val = vSpectrum.b[i] * (ti->rf[i].Height - 1) / vBarSize[i];

				if(val >= ti->rf[i].Height) val = ti->rf[i].Height -1;
				w = ti->rf[i].Height - val;
				if(w < 0) w = 0;
				x = w;
				ti->graph->SetClip(ti->rf[i], CombineModeReplace);
