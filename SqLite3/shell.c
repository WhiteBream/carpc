    ...
    case MODE_Column: {
      if( p->cnt++==0 ){
        for(i=0; i<nArg; i++){
          ...
          if( w<=0 ){
            w = (int)strlen(azCol[i] ? azCol[i] : "");
            if( w<10 ) w = 10;
            n = (int)strlen(azArg && azArg[i] ? azArg[i] : p->nullvalue);
            if( w<n ) w = n;
          }
...
static char * appendText(char *zIn, char const *zAppend, char quote){
  int len;
  int i;
  int nAppend = (int)strlen(zAppend);
...
static void process_input(struct callback_data *p, FILE *in){
  ...
  while( fflush(p->out), (zLine = one_input_line(zSql, in))!=0 ){
    ...
    if( zSql==0 ){
      ...
      if( zLine[i]!=0 ){
        nSql = (int)strlen(zLine);
        zSql = malloc( nSql+1 );
        strcpy(zSql, zLine);
      }
    }else{
      int len = (int)strlen(zLine);
